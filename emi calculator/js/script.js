...
function updateLoanDurationInputValue(value, period) {
  ...
  if (period === "month") {
    ...
    console.log(loanDurationInput.value);
  } else if (period === "year") {
    loanDurationInput.value = value / 12;
    console.log(loanDurationInput.value);
  }
  currentLoanDurationValue = value;
  console.log("currentLoanDuration:", currentLoanDurationValue);
}
...
// rate of interest
let loanInterestInput = document.getElementById("loan__interest--input");
...
function calculateAndDisplayEMI() {
  ...
  ).innerText = `Total amount payable ₹${totalAmountPayable.toFixed(2)}`;
  document.getElementById(
    "total_interest_dynamic"
  ).innerText = `total interest payable ₹${totalInterestPayable.toFixed(2)}`;
...
function createChart() {
  ...
  //   "₹" + totalAmountPayable.toFixed(0) + "*";
  // document.getElementById("total_interest_dynamic").innerHTML =
  //   "₹" + totalInterestPayable.toFixed(0) + "*";
  calculateAndDisplayEMI();
  // document.getElementById("resultEMI").innerText = "₹" + emiUI.toFixed(0) + "*";
}
